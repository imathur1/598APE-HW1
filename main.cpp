...
Texture *parseTexture(FILE *f, bool allowNull) {
  ...
  if (streq(texture_type, "color")) {
    ...
    if (lscanf(f, "%d %d %d %lf %lf %lf\n", &r, &g, &b, &opacity, &reflection,
               &ambient) == EOF) {
      printf("Could not read <r> <g> <b> <opacity> <reflection> <ambient>\n");
      exit(1);
    }
    return new ColorTexture((unsigned char)r, (unsigned char)g,
                            (unsigned char)b, opacity, reflection, ambient);
  }
    ...
    while (lscanf(f, "%s", object_type) != EOF) {
      if (streq(object_type, "light")) {
        ...
        if (lscanf(f, "%lf %lf %lf %lf %lf %lf %lf %lf\n", &plane_x, &plane_y,
                   &plane_z, &yaw, &pitch, &roll, &tx, &ty) == EOF) {
        ...
        double tx, ty;
        if (lscanf(f, "%lf %lf %lf %lf %lf %lf %lf %lf\n", &disk_x, &disk_y,
                   &disk_z, &yaw, &pitch, &roll, &tx, &ty) == EOF) {
        ...
        double tx, ty;
        if (lscanf(f, "%lf %lf %lf %lf %lf %lf %lf %lf\n", &box_x, &box_y,
                   &box_z, &yaw, &pitch, &roll, &tx, &ty) == EOF) {
        ...
        if (lscanf(f, "%lf %lf %lf %lf %lf %lf %lf %lf %lf\n", &x1, &y1, &z1,
                   &x2, &y2, &z2, &x3, &y3, &z3) == EOF) {
          printf(
              "Could not read <x1> <y1> <z1> <x2> <y2> <z2> <x3> <y3> <z3>\n");
          exit(1);
        }
        Texture *texture = parseTexture(f, false);
        ...
        if (lscanf(f, "%lf %lf %lf %lf %lf %lf %lf\n", &sphere_x, &sphere_y,
                   &sphere_z, &yaw, &pitch, &roll, &radius) == EOF) {
        ...
        if (lscanf(f, "%s %d %s %d %lf %lf %lf\n", point_filepath, &num_points,
                   poly_filepath, &num_polygons, &off_x, &off_y,
                   &off_z) == EOF) {
...
int main(int argc, const char **argv) {
  ...
  for (int i = 1; i < argc; i++) {
    ...
    if (streq(argv[i], "-F")) {
      if (i + 1 >= argc) {
        printf(
            "Error -F option must be followed by an integer number of frames");
      }
    ...
    if (streq(argv[i], "-a")) {
      if (i + 1 >= argc) {
        printf(
            "Error -a option must be followed by an animation input file path");
      }
