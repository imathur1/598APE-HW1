...
Camera::Camera(const glm::dvec3 &cente)
    : focus(cente), right(cente), up(cente), forward(cente) {
  setAngles(0, 0, 0);
}

Camera::Camera(const glm::dvec3 &f, double a, double b, double c)
    : focus(f), right(f), up(f), forward(f) {
  setAngles(a, b, c);
}
